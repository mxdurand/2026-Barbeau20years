...
df <- fread("data/B20Y_EC_all.csv")
df[df == -9999] <- NA
df <- df[, c(1:8, 10:19, 26, 30, 36, 38, 39, 43, 46:51, 60)]
colnames(df) <- c("y", "m", "d", "h", "doy", "par", "rg", "ta", "rh", "vpd", "press", "p", "ws", "ustar", "fc", "sc", "co2", "nee", "gpp", "ter", "le", "h2o", "e", "he", "rn", "swin", "swout", "lwin", "lwout", "gmoy", "etp")
...
# Conversions
df[, press := press / 10] # in kPa
df[, del := 4098 * (0.6108 * exp(17.27 * ta / (ta + 237.3))) / (ta + 237.3)^2] # Slope of saturation vapour pressure curve (before conversion to celsius)
df[, ta := ta + 273.15]   # in Kelvins
df[, gam := (cpa * press) / (eps * lam)]
df[, ws := ws * 4.87 / log(67.8 * 37 - 5.42)]

# Soil heat flux 
df[, g  := gmoy * 1800 / 1e6]     # in MJ/m2 per half hour [1800 seconds in 30 min]
df[, rn := rn * 1800 / 1e6]       # in MJ/m2 per half hour [1800 seconds in 30 min]
df[is.na(g), g := 0.05 * rn]   # rn now in MJ, so this is consistent

# Calculate ETP
df[, ETP_mm := (0.408 * del * (rn - g) + gam * (37 / ta) * ws * vpd) / (del + gam * (1 + 0.34 * ws))] # in mm/half-hour
df[, ETP := ETP_mm / 1800 * lam]

df[ETP < 0, ETP := 0]
df[ETP_mm < 0, ETP_mm := 0]
df[etp < 0, etp := 0]

# plot(ETP ~ etp, df, xlim = c(0,1200), ylim = c(0,1200), pch = 16, cex = 0.3, col = rgb(0, 0, 0, 0.1))
# abline(0, 1, col = "red")
# summary(lm(ETP ~ etp, df))

# ET / PET ratio
df[, etpet := e / ETP_mm]

df[ETP_mm <= 0.001, etpet := NA]   # avoid division by near-zero
df[rn <= 0, etpet := NA]           # nighttime meaningless
df[etpet < 0, etpet := NA]        # negative ET is not meaningful here
...
dfJ[, etpet_sum := e / etp] # Daily sum more accurate perhaps

# Define growing season by GPP
GPP_max <- dfJ[, .(gppmax = max(gpp, na.rm = TRUE)), by = y]
...
plot(e ~ y, dfY, cex = 1.4, bg = "gray", pch = 21)
plot(etp ~ y, dfY, cex = 1.4, bg = "gray", pch = 21)
plot(etpet ~ y, dfY, cex = 1.4, bg = "gray", pch = 21)

mE     <- lm(e ~ y, dfY)
mETP   <- lm(etp ~ y, dfY)
mETPET <- lm(etpet ~ y, dfY)
summary(mE)
summary(mETP)
summary(mETPET)
